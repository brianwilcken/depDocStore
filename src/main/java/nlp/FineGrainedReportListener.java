...
public abstract class FineGrainedReportListener {
    ...
    private final FineGrainedReportListener.Stats stats = new FineGrainedReportListener.Stats();
    ...
    public static class MatrixLabelComparator implements Comparator<String> {
        ...
        private Map<String, FineGrainedReportListener.ConfusionMatrixLine> confusionMatrix;

        public MatrixLabelComparator(Map<String, FineGrainedReportListener.ConfusionMatrixLine> confusionMatrix) {
        ...
        public int compare(String o1, String o2) {
            ...
            FineGrainedReportListener.ConfusionMatrixLine t1 = confusionMatrix.get(o1);
            FineGrainedReportListener.ConfusionMatrixLine t2 = confusionMatrix.get(o2);
            if (t1 == null || t2 == null) {
                if (t1 == null) {
                    return 1;
                } else {
    ...
    public static class GroupedMatrixLabelComparator implements Comparator<String> {
        ...
        public GroupedMatrixLabelComparator(Map<String, FineGrainedReportListener.ConfusionMatrixLine> confusionMatrix) {
            this.confusionMatrix = confusionMatrix;
            this.categoryAccuracy = new HashMap<>();

            // compute grouped categories
            for (Entry<String, FineGrainedReportListener.ConfusionMatrixLine> entry : confusionMatrix.entrySet()) {
                final String key = entry.getKey();
                final FineGrainedReportListener.ConfusionMatrixLine confusionMatrixLine = entry.getValue();
    ...
    public Comparator<String> getMatrixLabelComparator(Map<String, FineGrainedReportListener.ConfusionMatrixLine> confusionMatrix) {
        return new FineGrainedReportListener.MatrixLabelComparator(confusionMatrix);
    }

    public static class SimpleLabelComparator implements Comparator<String> {

        private Map<String, FineGrainedReportListener.Counter> map;

        public SimpleLabelComparator(Map<String, FineGrainedReportListener.Counter> map) {
            this.map = map;
        }
    ...
    public Comparator<String> getLabelComparator(Map<String, FineGrainedReportListener.Counter> map) {
        return new FineGrainedReportListener.SimpleLabelComparator(map);
    }
    ...
    public static class GroupedLabelComparator implements Comparator<String> {
        ...
        private Map<String, FineGrainedReportListener.Counter> labelCounter;

        public GroupedLabelComparator(Map<String, FineGrainedReportListener.Counter> map) {
            this.labelCounter = map;
            this.categoryCounter = new HashMap<>();

            // compute grouped categories
            for (Entry<String, FineGrainedReportListener.Counter> entry : labelCounter.entrySet()) {
                final String key = entry.getKey();
                final FineGrainedReportListener.Counter value = entry.getValue();
                final String category;
                if (key.contains("-")) {
                    category = key.split("-")[0];
                } else {
        ...
        public int compare(String o1, String o2) {
            ...
            if (c1.equals(c2)) { // same category - sort by confusion matrix
                ...
                FineGrainedReportListener.Counter t1 = labelCounter.get(o1);
                FineGrainedReportListener.Counter t2 = labelCounter.get(o2);
                if (t1 == null || t2 == null) {
                    if (t1 == null) {
                        return 1;
                    } else {
    ...
    public static class ConfusionMatrixLine {
        ...
        private Map<String, FineGrainedReportListener.Counter> line = new HashMap<>();
        private String ref;
        private int total = 0;
        ...
        /**
         * Creates a new {@link FineGrainedReportListener.ConfusionMatrixLine}
         *
         * @param ref
        ...
        private void increment(String column) {
            ...
            if (!line.containsKey(column)) {
                line.put(column, new FineGrainedReportListener.Counter());
            }
        ...
        public int getValue(String column) {
            FineGrainedReportListener.Counter c = line.get(column);
            if (c == null)
                return 0;
            return c.value();
        }
    ...
    public class Stats {
        ...
        private final Map<String, FineGrainedReportListener.Counter> tokOcurrencies = new HashMap<>();
        private final Map<String, FineGrainedReportListener.Counter> tokErrors = new HashMap<>();
        // tag statistics
        private final Map<String, FineGrainedReportListener.Counter> tagOcurrencies = new HashMap<>();
        private final Map<String, FineGrainedReportListener.Counter> tagErrors = new HashMap<>();
        private final Map<String, FMeasure> tagFMeasure = new HashMap<>();
        // represents a Confusion Matrix that aggregates all tokens
        private final Map<String, FineGrainedReportListener.ConfusionMatrixLine> generalConfusionMatrix = new HashMap<>();
        // represents a set of Confusion Matrix for each token
        private final Map<String, Map<String, FineGrainedReportListener.ConfusionMatrixLine>> tokenConfusionMatrix = new HashMap<>();
        ...
        private void commit(String tok, String ref, String pred) {
            ...
            if (!tokAccuracies.containsKey(tok)) {
                ...
                tokOcurrencies.put(tok, new FineGrainedReportListener.Counter());
                tokErrors.put(tok, new FineGrainedReportListener.Counter());
            }
            tokOcurrencies.get(tok).increment();

            // tag stats
            if (!tagOcurrencies.containsKey(ref)) {
                tagOcurrencies.put(ref, new FineGrainedReportListener.Counter());
                tagErrors.put(ref, new FineGrainedReportListener.Counter());
            }
            ...
            if (!generalConfusionMatrix.containsKey(ref)) {
                generalConfusionMatrix.put(ref, new FineGrainedReportListener.ConfusionMatrixLine(ref));
            }
            ...
            if (!tokenConfusionMatrix.get(tok).containsKey(ref)) {
                tokenConfusionMatrix.get(tok).put(ref, new FineGrainedReportListener.ConfusionMatrixLine(ref));
            }
        ...
        private double[][] createConfusionMatrix(SortedSet<String> tagset,
                                                 Map<String, FineGrainedReportListener.ConfusionMatrixLine> data) {
