...
{
    int [] gramSizes = null;
    TreeSet<String> validMultiGrams;

    public TokenSequenceNGrams (int [] sizes, TreeSet<String> validMultiGrams)
    {
        this.gramSizes = sizes;
        this.validMultiGrams = validMultiGrams;
    }
    ...
    public Instance pipe (Instance carrier)
    {
        ...
        for (int i = 0; i < ts.size(); i++) {
            ...
            for(int j = 0; j < gramSizes.length; j++) {
                ...
                if (validMultiGrams.contains(newTerm)) {
                    tmpTS.add(newTerm);
                }
            }
        }
