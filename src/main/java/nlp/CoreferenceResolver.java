...
public class CoreferenceResolver {
    ...
    public Collection<CorefChain> getCoreferences() {
        if (document != null) {
    ...
    public List<Coreference> getCoreferencesFromDocument(String text, String docId, List<NamedEntity> entities) {
        ...
        for (CorefTask corefTask : completedTasks) {
            ...
            for (CorefChain coref : corefs) {
                ...
                //If this is the case then each mention in the chain should be recorded and cross-referenced against a named entity.
                //It is the coreferences that will be used in the information extraction step.

                //Get the set of entities that exist in the current coreference partition
                List<NamedEntity> partitionEntities = entities.stream()
                        .filter(p -> corefTask.getLineStart() <= p.getLine() &&
                        p.getLine() < (corefTask.getLineStart() + partitionSize))
                        .collect(Collectors.toList());

                //For the current coreference chain find the set of partition entities that are similar to a given mention
                List<CorefChain.CorefMention> chainMentions = coref.getMentionsInTextualOrder();
                Map<CorefChain.CorefMention, List<NamedEntity>> validMentions = new HashMap<>();
                for (CorefChain.CorefMention mention : chainMentions) {
                    validMentions.put(mention, new ArrayList<>());
                    for (NamedEntity entity : partitionEntities) {
                        if (NLPTools.similarity(entity.getEntity(), mention.mentionSpan) > similarityThreshold) {
                            validMentions.get(mention).add(entity);
                        }
                    }
                }

                //Find the best entity match for each coreference mention
                for (Map.Entry<CorefChain.CorefMention, List<NamedEntity>> entry : validMentions.entrySet()) {
                    int lineNum = entry.getKey().sentNum - 1;
                    Optional<NamedEntity> closest = entry.getValue().stream()
                            .min(Comparator.comparingInt(p -> Math.abs(p.getLine() - lineNum)));
                    if (closest.isPresent()) {
                        List<NamedEntity> bestMatch = new ArrayList<>();
                        bestMatch.add(closest.get());
                        entry.setValue(bestMatch);
                    }
                }

                //For any coreference mentions that could not be automatically assigned a named entity find the best named entity association
                //Split apart the set of valid coreference mentions into those that are (un)associated with a named entity
                Map<CorefChain.CorefMention, List<NamedEntity>> unassociatedMentions = validMentions.entrySet().stream()
                        .filter(p -> p.getValue().size() == 0)
                        .collect(Collectors.toMap(p -> p.getKey(), q -> q.getValue()));

                Map<CorefChain.CorefMention, List<NamedEntity>> associatedMentions = validMentions.entrySet().stream()
                        .filter(p -> p.getValue().size() > 0)
                        .collect(Collectors.toMap(p -> p.getKey(), q -> q.getValue()));

                //In order to find the best matching coreference mention to use as a source for a named entity for each unassociated mention
                //find the associated mention with a line number closest.
                for (Map.Entry<CorefChain.CorefMention, List<NamedEntity>> entry : unassociatedMentions.entrySet()) {
                    Optional<CorefChain.CorefMention> closest = associatedMentions.keySet().stream()
                            .min(Comparator.comparingInt(p -> Math.abs(p.sentNum - entry.getKey().sentNum)));
                    if (closest.isPresent()) {
                        entry.getValue().addAll(associatedMentions.get(closest.get()));
                    }
                }

                //Add all coreference mentions in either set (since both sets should now contain only associated entities) to the finalized set of coreferences
                associatedMentions.putAll(unassociatedMentions);
                for (Map.Entry<CorefChain.CorefMention, List<NamedEntity>> entry : associatedMentions.entrySet()) {
                    if (entry.getValue().size() > 0) {
                        CorefChain.CorefMention mention = entry.getKey();
                        NamedEntity namedEntity = entry.getValue().get(0);
                        int lineNum = corefTask.getLineStart() + (mention.sentNum - 1);
                        Coreference coreference = new Coreference(docId, namedEntity, mention.mentionSpan, lineNum, mention.startIndex, mention.endIndex);
