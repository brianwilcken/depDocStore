...
public class CoreferenceResolver {
    ...
    public List<SolrDocument> getCoreferencesFromDocument(String text, String docId, List<NamedEntity> entities) {
        ...
        List<SolrDocument> corefEntities = new ArrayList<>();
        for (CorefTask corefTask : corefTasks) {
            ...
            for (CorefChain coref : corefs) {
                ...
                String entity = mention.toString().toLowerCase();
                List<NamedEntity> matchingEntities = entities.stream()
                        .filter(p -> (p.getEntity().toLowerCase().contains(entity) || entity.contains(p.getEntity().toLowerCase())) &&
                ...
                if (matchingEntities.size() > 0) {
                    ...
                    for (CorefChain.CorefMention corefMention : mentions) {
                        ...
                        doc.addField("coref", corefMention.mentionSpan);
                        doc.addField("line", corefTask.getLineStart() + corefMention.sentNum);
                        doc.addField("start", corefMention.startIndex);
                        doc.addField("end", corefMention.endIndex);

                        corefEntities.add(doc);
                    }
                }
            }
        }
